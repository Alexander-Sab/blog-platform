...
export function ContentList({ articles }) {
  ...
  useEffect(() => {
    setIsFavorited(articles.favorited)
  }, [articles.favorited])

  useEffect(() => {
    const savedIsFavorited = localStorage.getItem(
      `isFavorited_${articles.slug}`,
    )
    if (savedIsFavorited !== null) {
      setIsFavorited(JSON.parse(savedIsFavorited))
    }
  }, [articles.slug])
  ...
  const handleLike = async () => {
    ...
    try {
      if (isFavorited) {
        await dispatch(unfavoriteArticle({ token, slug: articles.slug })).then(
          () => {
            setIsFavorited(false)
          },
        )
      } else {
        await dispatch(favoriteArticle({ token, slug: articles.slug })).then(
          () => {
            setIsFavorited(true)
          },
        )
  ...
  console.log('articles.favorited', articles.favorited)
            ...
            >
              <button
                ...
                className={clsx(
                  ...
                  {
                    [classes[
                      'contentList-PostHeader___content-like-button-liked'
                    ]]: isFavorited,
                  },
                )}
              >
